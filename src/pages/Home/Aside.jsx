...
const Aside = () => {
    ...
    const addCategory = () => {
        ...
        axios.patch(`/users/${user.id}`, {categories: [...user.categories, newCategory]})
            .then(({data}) => {
                setUser({
                    ...data,
                    token: user.token
...
<path
    ...
    fill="#7C7C7C"/>
            ...
            <ul className="aside__menu">
                {
                    user.categories.map(({categoryName, color, id}) => (
                        <li key={id} className={`aside__item ${status === categoryName ? 'active' : ''}`} onClick={() => setStatus(categoryName)}>
                            <span style={{background: color}} className="aside__color"></span>
                            <span className="aside__text">{categoryName}</span>

                            <span className="aside__item-del" onClick={(e) => {
                                e.stopPropagation()
                                delCategory(id)
                            }}>
...
<path d="M1 6H11" stroke="#868686" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                ...
                <div style={{display: active ? 'block' : 'none'}} className='aside__popup'>
                    <input value={category} onChange={(e) => setCategory(e.target.value)} type="text"
                           className='aside__field' placeholder='Название категории'/>
                    <div className='aside__colors'>
                        {dataColors.map((item) => (
                            <span onClick={() => setColor(item)} key={item} className="aside__col"
                                  style={{background: item, border: color === item ? '3px solid black' : 'none'}}/>
                        ))}
                    </div>
                    <button type='button' className="aside__add" onClick={checkCategories}>Добавить</button>
