...
const ContentCategory = ({statusContent}) => {
    ...
    const addTask = (data) => {
        ...
        axios.patch(`/users/${user.id}`, {categories: newCategories})
            .then(({data}) => {
                setUser({
                    ...data,
                    token: user.token
    ...
    const checkTasks = (data) => {
        let hasTask = user.categories.find((item) =>
            item.categoryName === status).tasks.findIndex((item) =>
    ...
    const handleChangeCategory = (data) => {
        ...
        let newArrCategories = user.categories.map(item =>
            item.categoryName === statusContent ? {...item, categoryName : data.categoryName} : item)


        axios.patch(`/users/${user.id}`, {categories: newArrCategories})
            .then((res) => {
                setUser({
                    ...res.data,
                    token: user.token
            ...
            .catch((err) => toast(`Название категории изменено!, ${err.message}`))
    }
    ...
    return (
        <>
            {
                showTitle ?
                    <form noValidate className="content__add" onSubmit={handleSubmit(checkCategoryName)}>
                    <input defaultValue={statusContent} {...register('categoryName', {
                        required: {
                            message: 'Название категории заполнить!',
                            value: true
                        },
                        minLength: {
                            value: 3,
                    ...
                    })} type="text" className="content__add-field" placeholder='Текст категории'/>
                    <span className="form__error">{errors.categoryName && errors.categoryName.message}</span>
                    <div className="content__add-action">
                        <button className="content__add-btn">Изменить</button>
                        <div className="content__add-close"  onClick={() => setShowTitle(false)}>Отмена</div>
                    </div>
                </form>
                    : <div className='content__top'>
                    <h2 className='content__title'>{statusContent === 'all' ? 'Все задачи' : statusContent}</h2>
                    <span className='content__edit' onClick={() => setShowTitle(true)}>
                   <svg width="15" height="15" viewBox="0 0 15 15" fill="#DFDFDF" xmlns="http://www.w3.org/2000/svg">
<path
...
/>
            ...
            <ul className='content__menu'>
                {
                    statusContent !== 'all' ? user.categories.find((item) =>
                        item.categoryName === statusContent).tasks.map((item) => (
                        <li className='content__item' key={item.id}>
                            <ContentCheckbox isComplete={item.isComplete} id={item.id} handleCompleteTask={handleCompleteTask}/>
                            {item.taskTitle}

                            <span className="content__item-del" onClick={() => delTask(item.id)}>
                        ...
                        show ?
                            <form noValidate className="content__add" onSubmit={handleSubmit(checkTasks)}>
                                <input {...register('taskTitle', {
                                    required: {
                                        message: 'Название задачи заполнить!',
                                        value: true
                                    },
                                    minLength: {
                                        value: 3,
                                ...
                                })} type="text" className="content__add-field" placeholder='Текст задачи'/>
...
<path d="M1 8H15" stroke="#B4B4B4" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                                ...
                                <p onClick={() => setShow(true)} className='content__bottom-text'>Новая задача</p>
                            </div>
